...
def create_realistic_test_image():
    ...
    # 簡単な顔（笑顔）
    draw.ellipse(
        [270, 100, 330, 160], fill="#FFDBAC", outline="#8B4513", width=2
    )  # 肌色
...
def test_realistic_image_upload_and_analysis():
    ...
    files = {"file": ("realistic_child_scene.png", test_image, "image/png")}
...
def test_streaming_with_realistic_image():
    ...
    latest_file = max(
        files, key=lambda f: os.path.getctime(os.path.join(uploads_dir, f))
    )
