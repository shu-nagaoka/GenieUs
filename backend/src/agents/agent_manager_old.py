...
class AgentManager:
    ...
    def __init__(self, tools: dict, logger: logging.Logger, settings, routing_strategy: RoutingStrategy | None = None):
        ...
        Args:
            ...
            settings: アプリケーション設定
            routing_strategy: ルーティング戦略（オプション）
    ...
    def _determine_agent_type(self, message: str) -> str:
        """強化されたルーティング決定ロジック（戦略パターン使用）"""
        if not self.routing_strategy:
            # 戦略パターンは必須（CompositionRootで常に注入される）
    ...
    async def _perform_specialist_routing(
        ...
        if agent_id and agent_id in self._agents:
            ...
            return await self._route_to_specific_agent_with_fallback(
                agent_id, message, user_id, session_id, conversation_history, family_info,
            )
    ...
    async def _check_and_route_to_specialist(
        ...
        specialist_agent, routing_info = self.routing_strategy._determine_specialist_agent(original_message.lower())
    ...
    async def route_query_async_with_info(
        ...
        try:
            ...
            if selected_agent_type in ["search_specialist", "outing_event_specialist"]:
                ...
                self.logger.info("🔍 Web検索ツールを利用してリアルタイム情報を取得します")
