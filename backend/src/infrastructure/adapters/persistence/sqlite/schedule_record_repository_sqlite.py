...
class ScheduleRecordRepository(ScheduleRecordRepositoryProtocol):
    ...
    async def create(self, schedule_event: ScheduleEvent) -> ScheduleEvent:
        ...
        """
        try:
            self.logger.info(f"🗄️ スケジュール記録DB作成: {schedule_event.title}")
            ...
            self.logger.info(f"✅ スケジュール記録DB作成完了: {schedule_event.event_id}")
    ...
    async def get_by_id(self, schedule_id: str) -> ScheduleEvent | None:
        ...
        """
        try:
            self.logger.debug(f"🔍 スケジュール記録DB取得: {schedule_id}")

            query = f"SELECT * FROM {self._table_name} WHERE id = ?"
            results = self.sqlite_manager.execute_query(query, (schedule_id,))
    ...
    async def update(self, schedule_event: ScheduleEvent) -> ScheduleEvent:
        ...
        """
        try:
            self.logger.info(f"📝 スケジュール記録DB更新: {schedule_event.event_id}")
            ...
            self.logger.info(f"✅ スケジュール記録DB更新完了: {schedule_event.event_id}")
            return schedule_event
    ...
    async def delete(self, schedule_id: str) -> bool:
        ...
        """
        try:
            self.logger.info(f"🗑️ スケジュール記録DB削除: {schedule_id}")

            query = f"DELETE FROM {self._table_name} WHERE id = ?"
            affected_rows = self.sqlite_manager.execute_update(query, (schedule_id,))

            success = affected_rows > 0
            if success:
                self.logger.info(f"✅ スケジュール記録DB削除完了: {schedule_id}")
            else:
    ...
    async def search(
        ...
        """
        try:
            self.logger.debug(f"🔍 スケジュール記録DB検索: user_id={user_id}")
            ...
            self.logger.debug(f"✅ スケジュール記録DB検索完了: {len(schedule_events)}件")
    ...
    async def get_by_user_id(self, user_id: str, limit: int = 50, offset: int = 0) -> list[ScheduleEvent]:
        ...
        """
        return await self.search(user_id=user_id, limit=limit, offset=offset)
    ...
    async def get_events_by_date(self, user_id: str, target_date: datetime) -> list[ScheduleEvent]:
        ...
        """
        return await self.search(user_id=user_id, start_date=target_date, end_date=target_date, limit=100)
    ...
    async def initialize_table(self) -> None:
        """テーブル初期化（開発・テスト用）"""
        try:
            self.logger.info(f"🗄️ スケジュール記録テーブル初期化: {self._table_name}")
            ...
            self.logger.info(f"✅ スケジュール記録テーブル初期化完了: {self._table_name}")
