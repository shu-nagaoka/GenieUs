...
class EffortReportRepository(EffortReportRepositoryProtocol):
    ...
    async def create(self, effort_report: EffortReportRecord) -> EffortReportRecord:
        """努力レポート作成
        ...
        """
        try:
            self.logger.info(f"🗄️ 努力レポートDB作成: user_id={effort_report.user_id}, score={effort_report.score}")
            ...
            self.logger.info(f"✅ 努力レポートDB作成完了: {effort_report.report_id}")
    ...
    async def get_by_id(self, report_id: str) -> EffortReportRecord | None:
        """ID指定で努力レポート取得
        ...
        """
        try:
            self.logger.debug(f"🔍 努力レポートDB取得: {report_id}")

            query = f"SELECT * FROM {self._table_name} WHERE report_id = ?"
            results = self.sqlite_manager.execute_query(query, (report_id,))
    ...
    async def get_by_user_id(
        ...
        """
        try:
            self.logger.debug(f"🔍 努力レポート一覧DB取得: user_id={user_id}")

            query = f"SELECT * FROM {self._table_name} WHERE user_id = ? ORDER BY created_at DESC"
    ...
    async def get_user_report(self, user_id: str, report_id: str) -> EffortReportRecord | None:
        """ユーザー権限付きで特定レポート取得
        ...
        try:
            ...
            query = f"SELECT * FROM {self._table_name} WHERE report_id = ? AND user_id = ?"
            results = self.sqlite_manager.execute_query(query, (report_id, user_id))
    ...
    async def update(self, effort_report: EffortReportRecord) -> EffortReportRecord:
        """努力レポート更新
        ...
        """
        try:
            self.logger.info(f"📝 努力レポートDB更新: {effort_report.report_id}")
            ...
            UPDATE {self._table_name} SET
                period_days = ?, effort_count = ?, score = ?,
                highlights = ?, categories = ?, summary = ?, achievements = ?,
                updated_at = ?
            WHERE report_id = ?
            """
            ...
            self.logger.info(f"✅ 努力レポートDB更新完了: {effort_report.report_id}")
            return effort_report
    ...
    async def delete(self, report_id: str) -> bool:
        ...
        """
        try:
            self.logger.info(f"🗑️ 努力レポートDB削除: {report_id}")

            query = f"DELETE FROM {self._table_name} WHERE report_id = ?"
            affected_rows = self.sqlite_manager.execute_update(query, (report_id,))

            success = affected_rows > 0
            if success:
                self.logger.info(f"✅ 努力レポートDB削除完了: {report_id}")
            else:
    ...
    async def delete_user_report(self, user_id: str, report_id: str) -> EffortReportRecord | None:
        """ユーザー権限付きで努力レポート削除
        ...
        try:
            ...
            # 削除前に取得して権限確認
            report_to_delete = await self.get_user_report(user_id, report_id)
            if not report_to_delete:
                self.logger.warning(f"⚠️ 削除権限なし/見つかりません: user_id={user_id}, report_id={report_id}")
                return None

            # 削除実行
            query = f"DELETE FROM {self._table_name} WHERE report_id = ? AND user_id = ?"
            affected_rows = self.sqlite_manager.execute_update(query, (report_id, user_id))
    ...
    async def get_all_reports(self, limit: int = 100, offset: int = 0) -> list[EffortReportRecord]:
        """全努力レポート取得（管理用）
    ...
    async def count_user_reports(self, user_id: str) -> int:
        """ユーザー別努力レポート件数取得
        ...
        """
        try:
            query = f"SELECT COUNT(*) FROM {self._table_name} WHERE user_id = ?"
            results = self.sqlite_manager.execute_query(query, (user_id,))
            return results[0]["COUNT(*)"] if results else 0
    ...
    async def initialize_table(self) -> None:
        """テーブル初期化（開発・テスト用）"""
        try:
            self.logger.info(f"🗄️ 努力レポートテーブル初期化: {self._table_name}")
            ...
            self.logger.info(f"✅ 努力レポートテーブル初期化完了: {self._table_name}")
