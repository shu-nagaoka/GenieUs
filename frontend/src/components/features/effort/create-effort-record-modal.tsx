...
export function CreateEffortRecordModal({ 
  open, 
  onOpenChange, 
  onSuccess 
}: CreateEffortRecordModalProps) {
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [newHighlight, setNewHighlight] = useState('')
  const [newAchievement, setNewAchievement] = useState('')
  
  const [formData, setFormData] = useState<EffortRecordCreateRequest>({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await createEffortRecord(formData)
      if (result.success) {
        onSuccess()
        onOpenChange(false)
        // フォームをリセット
  ...
  const updateCategory = (category: keyof typeof formData.categories, value: number) => {
    setFormData(prev => ({
      ...prev,
      categories: {
        ...prev.categories,
              ...
              {[
                ...
                <div key={key} className="space-y-2">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-bold text-gray-800">{formData.categories[key as keyof typeof formData.categories]}%</span>
                  </div>
                  <Input
                    type="range"
                    min="0"
                    ...
                    onChange={(e) => updateCategory(key as keyof typeof formData.categories, parseInt(e.target.value))}
                    className="w-full"
                  />
                  <div className="h-2 bg-gray-200 rounded-full">
                    <div 
                      className={`h-2 bg-gradient-to-r ${color} rounded-full transition-all duration-300`}
                      style={{ width: `${formData.categories[key as keyof typeof formData.categories]}%` }}
                    ></div>
                  </div>
                </div>
              ))}
